...
def plot_fig7_macro_vs_segm_mean(root_folder:str, output_path:str, upload_overleaf: bool = False):
        ...
        df_segm_cov_perc = df_segm_cov[(df_segm_cov["n"]<=250)&(df_segm_cov["method"]=="percentile")]
        df_segm_width_perc = df_segm_width[(df_segm_width["n"]<=250)&(df_segm_width["method"]=="percentile")]
        ...
        if upload_overleaf:
            upload_to_overleaf(output_path, f"Preprint/main_figs/{os.path.basename(output_path)}", commit_msg="Update Fig 7 Macro vs Segmentation mean")

def main():
    parser = argparse.ArgumentParser(description="Generate Figure 6 classification macro vs mean of segmentation metrics.")
    ...
    plot_fig7_macro_vs_segm_mean(root_folder, output_path, upload_overleaf=args.upload_overleaf)
