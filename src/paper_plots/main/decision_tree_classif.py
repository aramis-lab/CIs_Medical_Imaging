...
OUTPUT_MD = Path("coverage_tree_classif.md")
...
records = []
for agreg in AGGREGATION:
    
    for metric in (
        globals()[f'METRICS_CLASSIF_{agreg}']
        
    ):
    
        if agreg=='MICRO':
            INPUT_DIR='../results_metrics_classif'
        else:
            INPUT_DIR='../results_metrics_classif_macro'
        file = os.path.join(INPUT_DIR, f"aggregated_results_{metric}.csv")
        print(file)
...
for (agreg, metric, n), g in df.groupby(
    ['agregation', 'metric', 'n']
):

    methods = METHODS_CLASSIF[:-1] if metric != 'accuracy' else METHODS_CLASSIF
    ...
    if len(recommended) > 0:
        ...
        status[(agreg, metric, n)] = recommended


    # 2) otherwise keep warnings
    elif len(warnings) > 0:

        status[(agreg, metric, n)] = warnings


    # 3) otherwise red cross
    else:

        status[(agreg, metric, n)] = {
            "__none__": ("❌", None)
        }
################################################################################
