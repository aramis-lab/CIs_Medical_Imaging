...
def extract_df_segm_both(folder_path:str, file_prefix:str, metrics:list[str], stats:list[str])->pd.DataFrame:
    all_values=[]
                ...
                for algo in algos: 
                    ...
                    for n in n_subset:  # Show only selected n values
                        ...
                        for method, col in method_dict.items():
                            for idx, val in enumerate(data_n[col]):
                                width_val = data_n[f'width_{method}'].iloc[idx] if f'width_{method}' in data_n.columns else None
                                all_values.append({
                                    'metric': metric,
                                    'stat': stat,
                                    'task': task, 
...
def plot_all_width_segm(root_folder: str, output_path: str):
    ...
    df_segm = extract_df_segm_both(folder_path_segm, file_prefix_segm, metrics_segm, stats_segm)

    df_segm = df_segm[(df_segm["coverage"] >= 0.90) & (df_segm["coverage"] <= 0.975)]
...
def main():
    ...
    output_path = args.output_path or os.path.join(root_folder, "clean_figs/supplementary/all_width_segm_no_fails.pdf")
