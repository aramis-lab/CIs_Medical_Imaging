...
def plot_all_width_segm(root_folder: str, output_path: str, upload_overleaf: bool = False):
    ...
    if upload_overleaf:
        upload_to_overleaf(output_path, f"Preprint/supp_figs/{os.path.basename(output_path)}", commit_msg=f"Update Supp Fig width of all methods and metrics for segmentation")
...
def main():
    ...
    output_path = args.output_path or os.path.join(root_folder, "clean_figs/supplementary/width_segm.pdf")

    plot_all_width_segm(root_folder, output_path, upload_overleaf=args.upload_overleaf)
