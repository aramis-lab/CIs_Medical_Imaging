...
def accuracy(correct_pred, average=None, axis=None): # Input is (batch_size, n_bootstrap, n_samples)
    return np.count_nonzero(correct_pred, axis=axis) / correct_pred.shape[axis]

def precision(tp, fp, average="micro", axis=None):# Input is (batch_size, n_classes, n_bootstrap, n_samples)

    if average == "micro":
        tp = np.count_nonzero(tp, axis=(1, 3))
        fp = np.count_nonzero(fp, axis=(1, 3))
        denom = tp + fp
        prec = tp / denom
        return np.where(denom > 0, prec, 0.0)
    
    tp = np.count_nonzero(tp, axis=3)
    fp = np.count_nonzero(fp, axis=3)

    class_prec = np.where((tp + fp) > 0, tp / (tp + fp), 0.0)
    return np.mean(class_prec, axis=1)

def recall(tp, fn, average="micro", axis=None):

    if average == "micro":
        tp = np.count_nonzero(tp, axis=(1, 3))
        fn = np.count_nonzero(fn, axis=(1, 3))
        denom = tp + fn
        rec = tp / denom
        return np.where(denom > 0, rec, 0.0)

    tp = np.count_nonzero(tp, axis=3)
    fn = np.count_nonzero(fn, axis=3)
    class_recall = np.where((tp + fn) > 0, tp / (tp + fn), 0.0)
    return np.mean(class_recall, axis=1)

def f1(tp, fp, fn, average="micro", axis=None):
...
def fbeta(tp, fp, fn, beta=1.0, average="micro", axis=None):
    beta2 = beta ** 2
...
def npv(tn, fn, average="micro", axis=None):

    if average == "micro":
        tn = np.count_nonzero(tn, axis=(1, 3))
        fn = np.count_nonzero(fn, axis=(1, 3))
        denom = tn + fn
        npv_score = tn / denom
        return np.where(denom > 0, npv_score, 0.0)
    
    tn = np.count_nonzero(tn, axis=3)
    fn = np.count_nonzero(fn, axis=3)

    class_npv = np.where((tn + fn) > 0, tn / (tn + fn), 0.0)
    return np.mean(class_npv, axis=1)

def sensitivity(tp, fn, average="micro", axis=None):
    return recall(tp, fn, average, axis=axis)

def specificity(tn, fp, average="micro", axis=None):
...
def balanced_accuracy(tp, fp, tn, fn, average="micro", axis=None):
...
def mcc(tp, fp, tn, fn, average="micro", axis=None):
...
def ap(y_true, y_scores, average="micro", axis=None):
    """
...
def auroc(y_score, y_true_bin, average="micro", axis=None):
    """
...
def get_metric_scipy_compatible(metric):
    metric_dict = {
        "accuracy": accuracy,
        "npv": npv,
        "ppv": precision,
